...
function Stackmanager:ingest(stacks, appWins, shouldClean) -- {{{
  ...
  for stackId, groupedWindows in pairs(stacks) do
    ...
    stackline.utils.each(stack.windows, function(win)
      ...
      win.otherAppWindows = stackline.utils.filter(appWins[win.app], function(w)
